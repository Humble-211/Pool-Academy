...
export function ServiceCard({ service, delay = 0 }: ServiceCardProps) {
  ...
  return (
    <motion.article
      ...
      whileHover={{ y: -4 }}
      className="relative glass rounded-2xl p-6 border border-white/5 hover:border-gold-500/20 transition-all duration-300 group flex flex-col h-full overflow-hidden"
    >
      {service.badge && (
      ...
      <div className="w-12 h-12 rounded-xl bg-gold-500/10 border border-gold-500/20 flex items-center justify-center mb-4 group-hover:bg-gold-500/20 group-hover:scale-105 transition-all">
      ...
      <Link
        href="/contact"
        className="mt-4 inline-flex items-center gap-2 text-sm text-gold-400 hover:text-gold-300 font-medium group/link transition-colors"
      >
        Book this service
